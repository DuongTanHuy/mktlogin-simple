...
const InputTab = ({ variable, setVariable, inputValidate, setInputValidate }) => {
  ...
  const selectFileCSV = (event, id) => {
    const file = event.target.files[0];
    const _clone = cloneDeep(variable);
    const _find = _clone.findIndex((i) => i.id === itemClicking);
    _clone[_find].file = file;
    _clone[_find].value = isElectron() ? file?.path : file?.name;
    setInputValidate(inputValidate.filter((i) => i !== _clone[_find].id));
    setVariable(_clone);
      ...
      <Stack spacing={2} pt={1} pr={2}>
        {variable && variable.length > 0 ? (
          ...
          <Typography textAlign="center" color="text.secondary">
            {t('task.editTask.input.noVariable')}
          </Typography>
        )}
      </Stack>
      <CustomPopover
