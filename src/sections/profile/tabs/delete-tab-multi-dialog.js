...
import { deleteMultiTabApi } from 'src/api/profile.api';

const DeleteTabMultiDialog = ({ open, onClose, profileIds, workspaceId, handleReLoadData }) => {
  ...
  const handleDeleteTab = async () => {
    try {
      ...
      if (workspaceId) {
        await deleteMultiTabApi(workspaceId, payload);
        // handleReLoadData();
  ...
  return (
    <ConfirmDialog
      open={open}
      onClose={handleClose}
      title={t('dialog.deleteTab.header')}
      closeButtonName={t('dialog.deleteBookmark.actions.cancel')}
      content={
        <Stack spacing={3}>
          <Typography variant="body1">{t('dialog.deleteTab.subheader')}</Typography>
...
export default DeleteTabMultiDialog;

DeleteTabMultiDialog.propTypes = {
  open: PropTypes.bool,
