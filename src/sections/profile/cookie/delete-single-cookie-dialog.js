...
const DeleteSingleCookieDialog = ({ open, onClose, id, workspaceId, handleResetData }) => {
  ...
  const handleDeleteCookie = async () => {
    try {
      ...
      if (workspaceId) {
        ...
        await deleteMultiCookieApi(workspaceId, payload);
        if (isElectron()) {
  ...
  return (
    <ConfirmDialog
      ...
      content={
        <Stack spacing={1}>
          <Typography>{t('dialog.deleteCookie.deleteSingle')}</Typography>
          <Stack
            direction="row"
            spacing={0.5}
            alignItems="center"
      ...
      action={
        ...
        >
          {t('dialog.deleteCookie.actions.delete')}
        </LoadingButton>
      }
    />
  );
...
export default DeleteSingleCookieDialog;

DeleteSingleCookieDialog.propTypes = {
  open: PropTypes.bool,
