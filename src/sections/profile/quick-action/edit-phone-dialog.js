...
const EditPhoneDialog = ({
  ...
  const handleConfirm = async () => {
    try {
      ...
      handleUpdateAfterEdit(profileId, 'phone', phone);
  ...
  return (
    <Dialog fullWidth maxWidth="sm" open={open} onClose={handleClose}>
      <DialogTitle sx={{ pb: 2 }}>{t('quickAction.phone.title')}</DialogTitle>
      <DialogContent
        sx={{
          pt: '8px!important',
        }}
      >
        <TextField
          fullWidth
          label={t('quickAction.phone.label')}
          placeholder={t('quickAction.phone.placeholder')}
          value={phone}
