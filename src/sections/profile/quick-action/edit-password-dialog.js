...
const EditPasswordDialog = ({
  ...
  const handleConfirm = async () => {
    try {
      ...
      handleUpdateAfterEdit(profileId, 'password', password);
  ...
  return (
    <Dialog fullWidth maxWidth="sm" open={open} onClose={handleClose}>
      <DialogTitle sx={{ pb: 2 }}>{t('quickAction.password.title')}</DialogTitle>
      <DialogContent
        sx={{
          pt: '8px!important',
        }}
      >
        <TextField
          fullWidth
          label={t('quickAction.password.label')}
          placeholder={t('quickAction.password.placeholder')}
          value={password}
